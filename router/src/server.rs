...
use crate::auth::{AuthExtension, setup, auth_interceptor};
...
async fn predict(
    infer: Extension<Infer>,
    authExt: Extension<AuthExtension>,
    req: Json<GenerateRequest>,
) -> Result<(HeaderMap, Json<GenerateResponse>), (StatusCode, Json<ErrorResponse>)> {

    if !authExt.is_logged()
    {
        tracing::info!("Inference request rejected. Reason: No JWT provided");
        let err_json = ErrorResponse {
            error: "You are not logged".to_owned(),
            error_type: "Authentification".to_owned(),
        };
